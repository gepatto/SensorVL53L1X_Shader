...
class Settings{
    ...
    public var rangeLow:UInt = 0;
    public var rangeHigh:UInt = 0;
    ...
    public function load():Settings {
        ...
        if(sys.FileSystem.exists("./config.json")){
            ...
            if( Reflect.hasField(config, "rangeLow")) rangeLow = cast(config.rangeLow, UInt);
            if( Reflect.hasField(config, "rangeHigh")) rangeHigh = cast(config.rangeHigh, UInt);
        }
